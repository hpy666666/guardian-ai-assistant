...
/* Disable MSD debug output (enable for troubleshooting) */
//#define MSD_TRACE
...
static rt_err_t rt_msd_init(rt_device_t dev)
{
    ...
    /* config spi */
    {
        ...
        cfg.max_hz = 1000 * 200; /* 200kbit/s for init (some cards need slower) */
    ...
    /* init SD card */
    {
        ...
        /* Give card time to stabilize after power-up sequence */
        rt_thread_mdelay(10);
        ...
        {
            /* Read OCR with retry - some cards need multiple attempts */
            int retry;
            for (retry = 0; retry < 5; retry++)
            {
                ...
                rt_thread_mdelay(10);  /* Small delay between retries */
            }

            if (result != RT_EOK)
            {
                MSD_DEBUG("[err] It maybe SD2.0 But it is Not response to CMD58! (tried %d times)\r\n", retry);
                goto _exit;
            }
