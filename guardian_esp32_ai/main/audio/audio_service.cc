...
void AudioService::Initialize(AudioCodec* codec) {
    ...
    /* Create Opus encoder (DTX + VBR enabled via AS_OPUS_ENC_CONFIG) */
    ...
    /* Create input resampler if needed */
    ...
    /* Instantiate audio processor based on Kconfig */
#ifdef CONFIG_USE_AUDIO_PROCESSOR
...
#ifdef CONFIG_USE_AFE_WAKE_WORD
    // Create and initialize the AFE wake word detector.
    // It runs its own detection task and fires the callback when triggered.
    auto ww = std::make_unique<AfeWakeWord>();
    if (ww->Initialize(codec)) {
        ww->OnWakeWordDetected([this](const std::string& wake_word) {
            if (callbacks_.on_wake_word_detected) {
                callbacks_.on_wake_word_detected(wake_word);
            }
        });
        wake_word_ = std::move(ww);
        ESP_LOGI(TAG, "Wake word detection ready");
    } else {
        ESP_LOGE(TAG, "Wake word initialisation failed — running without wake word");
    }
#endif
...
#ifdef CONFIG_USE_AFE_WAKE_WORD
    // Start wake word detection immediately so the device is always listening
    // for the wake word when idle.
    if (wake_word_) {
        wake_word_->Start();
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_encode_queue_.empty() && audio_send_queue_.size() < MAX_SEND_PACKETS_IN_QUEUE) {
            ...
            if (opus_encoder_ != nullptr && (int)task->pcm.size() == encoder_frame_size_) {
                ...
                ESP_LOGE(TAG, "Encoder not ready or invalid frame size (got %u, expected %d)",
...
void AudioService::SetDecodeSampleRate(int sample_rate, int frame_duration) {
    ...
    auto ret = esp_opus_dec_open(&opus_dec_cfg, sizeof(esp_opus_dec_cfg_t), &opus_decoder_);
    if (opus_decoder_ == nullptr) {
        ESP_LOGE(TAG, "Failed to recreate decoder, error code: %d", ret);
        return;
    }
...
void AudioService::EnableAudioTesting(bool enable) {
    /* Not used in guardian, stub kept for interface compatibility */
}
...
void AudioService::EnableWakeWord(bool enable) {
#ifdef CONFIG_USE_AFE_WAKE_WORD
...
void AudioService::PlaySound(const std::string_view& ogg) {
    ...
    demuxer->OnDemuxerFinished([this](const uint8_t* data, int sample_rate, size_t sz) {
        auto packet = std::make_unique<AudioStreamPacket>();
        packet->sample_rate    = sample_rate;
        packet->frame_duration = 60;
        packet->payload.resize(sz);
        std::memcpy(packet->payload.data(), data, sz);
...
void AudioService::CheckAndUpdateAudioPowerState() {
    ...
    if (input_elapsed  > AUDIO_POWER_TIMEOUT_MS && codec_->input_enabled())  codec_->EnableInput(false);
