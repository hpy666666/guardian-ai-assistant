...
async def _do_send_audio(conn: "ConnectionHandler", opus_packet, flow_control):
    ...
    if conn.conn_from_mqtt_gateway:
        ...
        # 封装 BinaryProtocol3 头部（4字节）：type(1B)+reserved(1B)+payload_size(2B BE)
        # ESP32 protocol v3 接收方需要此头部才能识别为音频帧
        import struct
        header = struct.pack(">BBH", 0x00, 0x00, len(opus_packet))
        await conn.websocket.send(header + opus_packet)
