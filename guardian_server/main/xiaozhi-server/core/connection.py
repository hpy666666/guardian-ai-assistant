...
class ConnectionHandler:
    ...
    async def _route_message(self, message):
        ...
        if isinstance(message, str):
            ...
            # 剥离 BinaryProtocol3 头部（4字节：type+reserved+payload_size）
            # ESP32 发送格式: [type(1B)][reserved(1B)][payload_size(2B BE)][opus...]
            # type=0 表示音频帧，需要去掉头部只保留 Opus 数据
            if len(message) > 4 and message[0] == 0x00:
                import struct
                payload_size = struct.unpack(">H", message[2:4])[0]
                opus_data = message[4:4 + payload_size]
                self.asr_audio_queue.put(opus_data)
            else:
                # 无头部或未知格式，直接传入
